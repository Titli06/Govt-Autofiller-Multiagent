...
snapshot — no DB, no real vision-LLM call. Exercises the full node order (form_schema
-> profile_lookup -> confidence_scorer) and the type_mismatch short-circuit
(SPEC-PHASE2.md §6.2)."""
...
def _snapshot():
    return {
        "full_name": [CandidateView("pf-1", "doc-1", "aadhaar", "Ravi Kumar", 0.95, "confirmed", _NOW)],
        "dob": [CandidateView("pf-2", "doc-1", "aadhaar", "1998-04-12", 0.96, "user_confirmed", _NOW)],
        "aadhaar_number": [
            CandidateView("pf-3", "doc-1", "aadhaar", "234123412346", 0.4, "needs_confirmation", _NOW)
        ],
    }


def _invoke(declared_form_type, classifier, snapshot=None):
    graph = build_graph()
    return graph.invoke(
        {
            "user_id": "u1",
...
def test_filled_field_carries_provisional_confidence_and_provenance():
    result = _invoke("income_certificate", lambda images, known: "income_certificate")
